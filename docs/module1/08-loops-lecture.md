...
9. Вкладені цикли
10. Корисні супутники циклів: `enumerate`, `zip`, агрегатні функції
...
`zip()` зупиняється, щойно закінчилася **найкоротша** з послідовностей.
...
- `enumerate()` дає індекс разом зі значенням, `zip()` перебирає кілька послідовностей паралельно.
